...
public class ServerMoveCoordinate : MonoBehaviour
{
    ...
    void UpdateLandmark(Dictionary<string, float>[] landmarks)
    {
        Debug.Log("Landmark updated.");
        var coords = landmarks[id];
        target = new Vector3(coords["x"], coords["y"], coords["z"]);
    }
    ...
    void Update()
    {
        ...
        target = GetComponent<LandmarksServer>().target;
